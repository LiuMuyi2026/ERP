    ...
    "workflow_stages": [
        {
            ...
            "steps": [
                {"key": "classify", "label": "1. 询盘归类", "desc": "与客户沟通询盘细节，按A/B/C/D级分类告知采购员。", "owner": "业务员", "builtin": True},
                {"key": "price_inquiry", "label": "2. 采购询价", "desc": "填写产品需求并提交至供应链管理，采购员完成询价后将最终报价发回。", "owner": "业务员填写 / 采购员询价", "builtin": True},
                {"key": "soft_offer", "label": "3. 报虚盘", "desc": "须在6小时内回复。邮件须同时抄送直属主管和业务经理。", "owner": "业务员", "builtin": True},
                {"key": "firm_offer", "label": "4. 报实盘", "desc": "货物规格/公差/材质/价格/货源/付款方式/物流均确认后方可回复实盘。", "owner": "业务员", "builtin": True},
            ],
        },
        {
            ...
            "steps": [
                {"key": "confirm_details", "label": "1. 确认合同细节", "desc": "与客户确认买方公司名称、地址、国家、合同数量/金额、付款方式等。", "owner": "业务员+单证员", "builtin": True},
                {"key": "draft_contract", "label": "2. 起草销售合同", "desc": "单证员起草合同，业务员初审。", "owner": "单证员起草 / 业务员初审", "builtin": True},
                {"key": "order_note", "label": "3. 新订单登记表", "desc": "业务员将合同预计费用填入新订单登记表并呈递给业务主管签署。", "owner": "业务员", "builtin": True},
                {"key": "sign_contract", "label": "4. 签订销售合同", "desc": "合同草稿连同新订单登记表呈递给业务主管签署并盖章。", "owner": "业务主管 / 业务经理（OA）", "builtin": True},
                {"key": "send_contract", "label": "5. 合同会签确认", "desc": "单证员确认合同已完成双方会签，并上传双方签字合同文件。", "owner": "单证员", "builtin": True},
            ],
        },
        {
            "key": "procurement",
            ...
            "steps": [
                {"key": "procurement_check", "label": "1. 判定采购条件", "desc": "定金到账，或收到清洁LC正本，或收到客户签字盖章的销售合同后方可采购。", "owner": "单证员报告", "builtin": True},
                {"key": "confirm_supplier", "label": "2. 确认供应商", "desc": "由业务主管在供应商名录中选定供应商，告知采购经理和单证员采购意向。", "owner": "业务主管决定", "builtin": True},
                {"key": "sign_purchase", "label": "3. 签订采购合同", "desc": "采购经理上传已签章采购合同，合同同步归档至供应商管理。", "owner": "采购经理", "builtin": True},
                {"key": "pay_deposit", "label": "4. 付款（定金）", "desc": "单证员将双方签字盖章合同、对方办款资料及高达支款单呈递给财务。", "owner": "单证员整理 / 财务付款", "builtin": True},
            ],
        },
        {
            "key": "booking",
            ...
            "steps": [
                {"key": "freight_inquiry", "label": "1. 海运询价", "desc": "在货物备妥前10-20天订舱。须比较≥2家货代的运费及港杂。", "owner": "业务员", "builtin": True},
                {"key": "booking", "label": "2. 订舱", "desc": "散货/集装箱订舱，须满足订舱条件。", "owner": "业务员+单证员", "builtin": True},
                {"key": "cost_confirm", "label": "3. 实际费用确认", "desc": "装运完成后，单证员核对费用与订舱时是否一致。", "owner": "单证员核对 / 业务员签字", "builtin": True},
            ],
        },
        {
            "key": "shipping",
            ...
            "steps": [
                {"key": "labels", "label": "1. 标签确认", "desc": "单证员制作SHIPPING MARK标签样本，业务员向客户确认。", "owner": "单证员制作 / 业务员确认", "builtin": True},
                {"key": "inspection", "label": "2. 验货", "desc": "业务员决定是否出厂检验，执行检验并拍照留档。", "owner": "业务员决定", "builtin": True},
                {"key": "packing_details", "label": "3. 索要货物明细", "desc": "业务员催要货物明细，单证员制作分箱明细发送给货代。", "owner": "业务员催促 / 单证员制作", "builtin": True},
                {"key": "warehouse_entry", "label": "4. 高达入库登记", "desc": "收到货物明细后，单证员在高达系统上做采购入库登记。", "owner": "单证员", "builtin": True},
                {"key": "pay_balance", "label": "5. 付尾款", "desc": "单证员走付款流程，出纳员付款。", "owner": "单证员+出纳员", "builtin": True},
                {"key": "delivery_notice", "label": "6. 送货通知", "desc": "打印送货通知，注明装车要求，由业务员及业务主管签字。", "owner": "业务员+业务主管签字", "builtin": True},
                {"key": "godad_billing", "label": "7. 高达开单", "desc": "单证员在发货当月完成高达系统开单。", "owner": "单证员", "builtin": True},
                {"key": "customs", "label": "8. 报关", "desc": "单证员制作报关材料发送货代报关。", "owner": "单证员", "builtin": True},
                {"key": "clearance_photos", "label": "9. 确认通关及照片", "desc": "确认通关情况，索要装箱/装船照片。", "owner": "业务员+单证员", "builtin": True},
                {"key": "shipment_notice", "label": "10. 装船通知", "desc": "单证员在开船后2个工作日内制作装船通知。", "owner": "单证员", "builtin": True},
                {"key": "documents", "label": "11. 单据制作与交单", "desc": "单证员制作全套议付单据及附件，交单后登记跟踪。", "owner": "单证员", "builtin": True},
            ],
        },
        {
            "key": "collection",
            ...
            "steps": [
                {"key": "follow_payment", "label": "1. 跟进回款", "desc": "业务员跟进非信用证回款，单证员跟进信用证回款。", "owner": "业务员（非LC）/ 单证员（LC）", "builtin": True},
                {"key": "risk_delivery", "label": "2. 风险放货审批", "desc": "后TT有应收时客户要求放货属风险放货，须向业务主管、业务经理、风控经理汇报。", "owner": "单证员报告", "builtin": True},
                {"key": "arrival_notice", "label": "3. 到港提醒", "desc": "业务员在货物到港前7天发邮件提醒客户做好接货准备。", "owner": "业务员", "builtin": True},
                {"key": "after_sales", "label": "4. 售后回访", "desc": "货物到港15天后发送客户满意度调查，跟踪使用情况和质量异议。", "owner": "业务员+单证员", "builtin": True},
                {"key": "filing", "label": "5. 档案归档", "desc": "所有单据和照片上传到微盘对应合同文件夹，纸质版存入档案柜。", "owner": "单证员+财务", "builtin": True},
            ],
        },
    ],
    "file_categories": [
