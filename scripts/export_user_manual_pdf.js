...
const mdPath = path.join(ROOT, 'docs', 'user-manual', '使用说明书-用户版.md');
const outHtml = path.join(ROOT, 'docs', 'user-manual', '使用说明书-用户版.html');
const outPdf = path.join(ROOT, 'docs', 'user-manual', '使用说明书-用户版.pdf');
const imgDir = path.join(ROOT, 'docs', 'user-manual', 'images');
...
function mdToHtml(md) {
  ...
  for (const raw of lines) {
    ...
    if (img) {
      ...
      const alt = esc(img[1]);
      const rel = img[2];
      const src = imgData(rel);
      out.push(`<figure><img src="${src}" alt="${alt}"/><figcaption>${alt}</figcaption></figure>`);
      continue;
    }
