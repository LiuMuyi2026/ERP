...
// ── LeadModal extracted to ./components/LeadModal.tsx ──────────────────────────
// (old inline definition removed — now imported from ./components/LeadModal)
            ...
            <div className="flex items-center gap-1 px-3 py-1.5 flex-shrink-0" style={{ background: '#f0f2f5', borderBottom: '1px solid #d1d7db' }}>
              {([['whatsapp', 'WhatsApp', '#00a884'], ['broadcast', 'Broadcast', '#7c3aed'], ['analytics', 'Analytics', '#3b82f6'], ['commlog', tCrm('subTabCommLog'), '#667781']] as const).map(([key, label, color]) => (
                <button key={key} onClick={() => setMsgSubTab(key as any)}
                  ...
                  {key === 'whatsapp' && (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill={msgSubTab === key ? '#00a884' : '#8696a0'}>
                  ...
                  {key === 'broadcast' && (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={msgSubTab === key ? '#7c3aed' : '#8696a0'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
